...
inline double KTRProblem::evaluateFC(const double* const x, double* const c, double* const objGrad, double* const jac) {
    return KTR_RC_CALLBACK_ERR;
}

inline int KTRProblem::evaluateGA(const double* const x, double* const objGrad, double* const jac) {
  return KTR_RC_CALLBACK_ERR;
}

inline int KTRProblem::evaluateHess(const double* const x, double objScaler, const double* const lambda, double* const hess) {

  return KTR_RC_CALLBACK_ERR;
}

inline int KTRProblem::evaluateHessianVector(const double* const x, double objScaler, const double* const lambda, double* const vector) {
...
inline int KTRProblem::mipNodeCallbackWrapper(int evalRequestCode, const double* const x, const double* const lambda, double obj,
  ...
  // MIP node callbacks are only available for KTRSolver, hence the cast here
  return _mipNodeCallback->CallbackFunction(x, lambda, obj, c, (KTRSolver*)solver);
}

inline int KTRProblem::msProcessCallbackWrapper(int evalRequestCode, const double* const x, const double* const lambda, double obj,
                                                double* const c, double* const objGrad, double* const jac,
                                                double* const hessian, double* const hessVector, KTRISolver* solver) {
  ...
  return _msProcessCallback->CallbackFunction(x, lambda, obj, c, solver);
}

inline int KTRProblem::msInitPtCallbackWrapper(int nSolveNumber, const double* const xLoBnds, const double* const xUpBnds,
                                               double* const x, double* const lambda, KTRISolver* solver) {
...
inline int KTRProblem::newPointCallbackWrapper(KTR_context_ptr kc, const double* const x, const double* const lambda,
                                               double obj, const double* const c, const double* const objGrad,
                                               const double* const jac, KTRISolver* solver) {
  if (_newPointCallback == NULL) {
    return KTR_RC_CALLBACK_ERR;
  }

  return _newPointCallback->CallbackFunction(x, lambda, 
                                             obj, c, objGrad, jac, solver);
}
