...
inline void KTRSolver::setCompCons() {
  ...
  int retCode = KTR_set_compcons(_kc, static_cast<int>(indexList1.size()), &indexList1[0], &indexList2[0]);
  if (retCode != 0) {
    std::ostringstream message;
    message << "Error returned from KTR_set_compcons";
...
inline int KTRSolver::callbackEvalFC(int evalRequestCode, int n, int m, int nnzJ, int nnzH, const double* const x,
  ...
  int status = solver->getProblem()->evaluateFC(x, *obj, c, objGrad, jac);
...
inline int KTRSolver::callbackMSProcess(int evalRequestCode, int n, int m, int nnzJ, int nnzH, double const* const x,
  ...
  return solver->getProblem()->msProcessCallbackWrapper(evalRequestCode, x, lambda, *obj, c, objGrad,
                                                        jac, hessian, hessVector, solver);
}
...
inline int KTRSolver::callbackMipNode(int evalRequestCode, int n, int m, int nnzJ, int nnzH, double const* const x,
  ...
  return solver->getProblem()->mipNodeCallbackWrapper(evalRequestCode, x, lambda, *obj, c, objGrad, jac,
                                                      hessian, hessVector, solver);
}
...
inline int KTRSolver::callbackNewPt(KTR_context_ptr kc, int n, int m, int nnzJ, double const* const x,
  ...
  return solver->getProblem()->newPointCallbackWrapper(kc, x, lambda, obj, c, objGrad, jac, solver);
}
...
inline int KTRSolver::callbackEvalGA(int evalRequestCode, int n, int m, int nnzJ, int nnzH, double const* const x,
  ...
  int returnVal = solver->getProblem()->evaluateGA(x, objGrad, jac);
...
inline int KTRSolver::callbackEvalHess(int evalRequestCode, int n, int m, int nnzJ, int nnzH, double const* const x,
  ...
  switch (evalRequestCode) {
    case KTR_RC_EVALH:
      returnCode = solver->getProblem()->evaluateHess(x, 1.0, lambda, hessian);
      break;
    case KTR_RC_EVALH_NO_F:
      returnCode = solver->getProblem()->evaluateHess(x, 0.0, lambda, hessian);
      break;
    case KTR_RC_EVALHV:
      returnCode = solver->getProblem()->evaluateHessianVector(x, 1.0, lambda, hessVector);
      break;
    case KTR_RC_EVALHV_NO_F:
      returnCode = solver->getProblem()->evaluateHessianVector(x, 0.0, lambda, hessVector);
      break;
    default:
